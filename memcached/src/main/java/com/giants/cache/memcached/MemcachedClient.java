...
public class MemcachedClient {
	...
	// logger
	private static Logger log = LoggerFactory.getLogger(MemcachedClient.class
			.getName());
	...
	private boolean set( String cmdname, String key, Object value, Date expiry, Integer hashCode, boolean asString ) {
        ...
        if ( NativeHandler.isHandled( value ) ) {
			...
			try {
				...
				this.serializeByHessian(bos, value);
				val = bos.toByteArray();
				flags |= F_SERIALIZED;
			}
			catch ( IOException e ) {
			...
			while ( true ) {
				...
				if ( line.startsWith( VALUE ) ) {
					...
					if ( ( flag & F_SERIALIZED ) != F_SERIALIZED ) {
						...
						Hessian2Input ois = new Hessian2Input(new ByteArrayInputStream(buf));
						ois.setSerializerFactory(serializerFactory);
						try {
							o = ois.readObject();
							ois.close();
							if ( log.isInfoEnabled() )
